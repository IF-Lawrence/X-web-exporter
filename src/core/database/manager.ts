  ...
  async extAddTweets(extName: string, tweets: Tweet[]) {
    ...
    if (dateRangeFilter.isActive()) {
      filteredTweets = tweets.filter((tweet) => {
        const timestamp = +parseTwitterDateTime(tweet.legacy.created_at);
        const withinRange = dateRangeFilter.isWithinRange(timestamp);

        if (!withinRange) {
          logger.debug(
            `Tweet ${tweet.rest_id} filtered out by date range: ${new Date(timestamp).toISOString()}`,
          );
        }
      ...
      const filtered = tweets.length - filteredTweets.length;
      if (filtered > 0) {
        logger.info(
          `Date filter: ${filtered} of ${tweets.length} tweets filtered out (${filteredTweets.length} accepted)`,
        );
      }
    }

    // Save filtered tweets
    await this.upsertTweets(filteredTweets);
