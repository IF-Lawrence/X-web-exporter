...
import { captureController } from '@/core/capture-controller';
  ...
  private installHttpHooks() {
    ...
    globalObject.XMLHttpRequest.prototype.open = function (method: string, url: string) {
      ...
      this.addEventListener('load', () => {
        ...
        // Run current enabled interceptors only when capturing is active.
        manager
          .getExtensions()
          .filter((ext) => ext.enabled && captureController.isCapturing)
          .forEach((ext) => {
    ...
    setTimeout(() => {
      if (!('webpackChunk_twitter_responsive_web' in globalObject)) {
        logger.error(
          ...
          'See: https://github.com/IF-Lawrence/Orbit-Exporter/issues/19',
        );
      }
    }, 1000);
  }
